 ...
 * 低层（annotation/naming/types/ddl）只向下；interceptor 是实现层（依赖 gen/model/config）；
 * gen 是顶层。模块方向由 Maven 自身强制，这里只约束包级依赖与循环。
 */
...
class ArchitectureTest {
    ...
    /** interceptor 是实现层：只允许依赖框架核心（gen/model/config），不得向上/横向蔓延。 */
    @ArchTest
    static final ArchRule interceptorOnlyDependsOnFramework = noClasses()
            .that()
            .resideInAPackage("hyc.codegen.core.interceptor..")
            .should()
            .dependOnClassesThat()
            .resideInAnyPackage(
                    "hyc.codegen.core.annotation..",
