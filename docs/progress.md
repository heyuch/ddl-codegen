...
- **2026-08-30：修复 rename 删旧文件导致手写代码丢失的缺陷**。原实现 RENAME = deleteArtifacts(旧表名) + 新表名生成——旧文件（含用户手写代码）被删除，违反「工具只增删改 @Generated 成员、用户手写代码永不触碰」核心契约（design.md 管线描述也写明 rename 应保留旧文件）。修复：rename 不再删除旧表名产物文件（保留手写代码），新表名正常生成；仅当类名变化（非 shard 后缀改名）时经 ChangeReport warning 提示手动迁移。测试：renameKeepsOldFilesAndGeneratesNew + renamePreservesUserWrittenCodeInOldFile（手写代码保留断言）。注意：类名变化的 rename 会产生旧/新两份文件（旧为孤儿，warning 引导迁移）——「不丢代码」优先于「无残留」，这是与「工具永不触碰用户代码」契约一致的取舍。
- **2026-08-30：DdlConfig lombok 化 + 全仓移除 java.util.Optional**。① DdlConfig 类级 @Getter/@Setter（7 组纯样板 getter/setter 删除；逻辑方法如 getRoot 判空、拷贝 getter 保留手写——lombok 不覆盖已存在方法）；② `artifact(name)` 返回 Optional → `@Nullable ArtifactConfig`，调用方改显式判空（GenerationContext 抽 requireArtifact、测试加 artifact 辅助），ArtifactRegistry.lookup 同步 @Nullable（无调用方预留 API）——项目禁用 Optional（用户偏好：可空性用 @Nullable 显式表达）；③ 顺带修复：EnumGenerator 枚举常量不设 type 导致 reconcile 路径触发 @MonotonicNonNull getType 判空 throw（alter 流程测试暴露）——枚举常量 type 设为所属枚举类型（语义正确 + signature 有值）。
