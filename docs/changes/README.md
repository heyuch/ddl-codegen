...
| [20260906-12-chore-design-quality-checklist](20260906-12-chore-design-quality-checklist/) | 完成 | **设计流程完善（07→11 教训 + GRASP + 模式取舍）**：SKILL 步骤 4「SOLID 自评」→「设计自评」并新增「设计自评检查单」四组——SOLID / GRASP 九原则（信息专家/创建者/低耦合/高内聚/控制器/多态/纯虚构/间接/预防变化，逐项一句话判据 + 项目实证锚点）/ 语义归层（方言只存在于直接生产者一层、模型存真值，反例实证 07→11）/ 设计模式取舍（能否用/哪种/简化/复杂/值得，不用模式也是结论；教训：不为简单开关建抽象）；步骤 5 独立评审同此视角；TEMPLATE「方案」「类职责与交互」补指针式提示；glossary 增 GRASP/设计模式两行；new-change.sh 提示同步；纯文档、自检合格 |
