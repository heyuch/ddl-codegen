...
| [20260906-13-feat-repository-spring-cache-crud](20260906-13-feat-repository-spring-cache-crud/) | 完成 | **Repository 写方法 CRUD + Spring Cache cache-aside 生成**：repository 接口/impl 补 insert/update/deleteById 透传（converter 双向 `to<Source>` 已存在，无 converter 改动）；`repository.springCache=true`（表有索引才生效）→ findBy* `@Cacheable`、`evictCaches`（`@Caching(evict={...})` 组 + log 行同 spec 循环产出、`key={method}:{args}`、cacheNames 缺省基类名）、写方法 fetch-first 老行/insert 回填行经 `@Autowired @Lazy` 自引用代理 evict、类去 final、NULL 入缓存防穿透；日志双形态 `repositoryImpl.lombok=true`→`@Slf4j` / 缺省→手写 Logger 常量置类顶；po import 简单名（冲突回退 FQN）；AbstractJavaGenerator 方法签名纳入注解比对（注解翻转触发 reconcile）；IT 样例开 springCache（内存缓存 + @EnableCaching + `-parameters` + SQL 计数拦截器断言命中/失效/防穿透）。实现期修正：Spring 5.3 `@CacheEvict` 不可重复→`@Caching` 容器；非 cache 透传漏 converter bug。fixture 先行多轮用户审查定稿（key 格式/日志形态/例名） |
...
- **历史类（2 问）**：
  1. 曾否决 / 曾作废的方案与教训？
  2. 最近几次变更改了什么？
  （答案以本索引行为准——含取代/作废注记；不逐份展开读历史变更目录）
- **卫生类（3 问）**：
...
- **对象与边界**：检查对象 = 记忆文档（AGENTS.md / SKILL.md / architecture.md / static-rules-review.md / 本索引 / TEMPLATE.md / glossary）+ 本次改动的当前变更目录（design/progress，一致性属步骤 9/10 本职）。**历史变更目录不在检查对象内**：其自检已在各自收尾时完成（收尾后不可变），无需重复自检；自检涉及历史时只**读取**作事实来源（对照索引行/被引用目录），不重新检查其文本。
- **执行规则**：任何记忆文档（同上集合）被改动后，由**新上下文实例只喂「对象与边界」所列对象作答**，答案与 architecture.md / 代码不符即**不合格**，修复后才算收尾。答错 = 要么文档没触发、要么文本已漂移。
