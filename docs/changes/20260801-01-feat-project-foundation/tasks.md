...
> ⚠️ **历史任务清单**：M0-M4 已完成，勾选状态不再更新；其中描述的机制可能与现行架构不符（如拦截器任务行已被 `2026-08-29-opt-core-first` 废除）——与现行架构冲突处以 `docs/architecture.md` 为准。
...
- [ ] 2.1 Generator SPI + AbstractJavaGenerator（@Generated 成员 reconcile 循环：
      缺→增 / 多余→删 / 签名类型不符→替换 / 一致→跳过）
- [ ] 2.2 GeneratorInterceptor SPI（只动 @Generated 成员 + import；幂等）+ 内置 lombok、jsr303
- [ ] 2.3 两阶段生成（阶段 A 发布描述 → 阶段 B 生成文件）+ GeneratorRegistry 兜底 +
