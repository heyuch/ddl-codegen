...
> ⚠️ **本文是 2026-08 的设计定稿历史基线（was）**：后续变更已使部分内容失效（拦截器/use 链已删除、config 与注解语义演进、`--force` 逃生口未实现等）。
> **现状一律以 `docs/architecture.md` 为准**（单一事实源，代码对账产出）；本文仅作历史溯源，勿作现状依据，冲突时以 architecture.md 为准。
>
> 原始说明：本文件是 DDL→Java 代码生成框架的完整技术设计，供实现 session 直接执行。
