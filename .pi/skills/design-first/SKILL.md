...
3. **设计**：填写 `design.md`（模板 `docs/changes/TEMPLATE.md` 的章节：背景与问题 / 可选方案与取舍 / 方案 / 改动文件与影响面 / 验证）
4. **评审（硬性关卡）**：把 `design.md` 呈现给用户评审；**未获认可不得进入实现**
5. **实现**：按设计写代码
6. **验证**：`JAVA_HOME=/opt/homebrew/opt/openjdk@11 mvn clean test` 全绿 + 静态检查全绿
7. **文档同步**：架构/数据流/约定变化 → 更新 `AGENTS.md`「项目架构」节或 `docs/design.md`；关键决策追加到 `docs/progress.md`「关键决策记录」
